...
class ScheduleConfig:
  noise_seed_train: int = 42
  noise_seed_val: int = 9999
...
def validate(cfg: Config) -> None:
  assert cfg.model.seq_len == cfg.data.seq_len, f'model.seq_len ({cfg.model.seq_len}) must equal data.seq_len ({cfg.data.seq_len})'
  assert cfg.train.batch_size % 1 == 0, 'batch_size must be a positive integer'
  assert cfg.model.d_model % cfg.model.n_heads == 0, 'd_model must be divisible by n_heads'
