...
def loss_fn(
  ...
  loss_per_pos = optax.softmax_cross_entropy_with_integer_labels(logits=logits, labels=labels)
  loss_weights = schedule.loss_weight(inputs['timesteps'])[:, None]
  loss_weights = jax.device_put(
    loss_weights,
    P(
      'data',
    ),
  )
  loss = (loss_weights * jnp.where(mask, loss_per_pos, 0.0)).sum() / jnp.maximum(n_masked, 1.0)
  return loss
...
def get_parameter_norm(model: DiffusionTransformer) -> jax.Array:
  param_norm = jnp.sqrt(sum(jnp.sum(x**2) for x in jax.tree.leaves(nnx.state(model, nnx.Param))))
  return param_norm


@partial(nnx.jit, static_argnames=('schedule',))
def train_step(
  model: DiffusionTransformer,
  ...
  if cfg.lr_schedule == 'none':
    ...
    schedule_fn = optax.join_schedules(
      schedules=[
        ...
        optax.cosine_decay_schedule(cfg.lr, cfg.max_steps - cfg.warmup_steps, alpha=cfg.min_lr / cfg.lr),
      ],
    ...
    schedule_fn = optax.join_schedules(
      schedules=[
        optax.linear_schedule(0.0, cfg.lr, cfg.warmup_steps),
        optax.linear_schedule(cfg.lr, cfg.lr, cfg.cooldown_start_steps - cfg.warmup_steps),
        optax.linear_schedule(cfg.lr, cfg.min_lr, cfg.max_steps - cfg.cooldown_start_steps),
      ],
    ...
    raise NotImplementedError('The kind of learning schedule specified is not implemented in `src/training.py`')
  return schedule_fn
...
def get_optimizer_kwargs(cfg: TrainConfig):
  ...
  if cfg.optimizer == 'adamw':
    ...
    raise NotImplementedError('The kind of optimizer specified is not implemented in `src/training.py`')
  return kwargs
...
def init_optimizer_alg(
  ...
  optimizer_ref = getattr(optax, cfg.optimizer) if cfg.optimizer != 'muon' else getattr(optax.contrib, cfg.optimizer)

  base_opt_alg = optax.chain(
    optax.clip_by_global_norm(cfg.clip_grad_norm) if cfg.clip_grad_norm > 0 else optax.identity(),
    optimizer_ref(learning_rate=schedule_fn, weight_decay=cfg.weight_decay, **optimizer_kwargs),
  )
