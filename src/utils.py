...
class MetricLogger:
  ...
  def save_logs(self, cfg: ExperimentConfig):
    save_folder = os.path.join(cfg.out_dir, cfg.run_name, f'step_{self.step_number}')
...
def load_checkpoint(cfg: Config, optimizer: nnx.Optimizer):
  ...
  abstract_model = nnx.eval_shape(lambda: DiffusionTransformer(cfg.model, rngs=nnx.Rngs(cfg.model.init_seed)))
  graphdef, abstract_state = nnx.split(abstract_model)
  model_state = checkpointer.restore(os.path.join(ckpt_folder, 'model_state'), abstract_state)
  model = nnx.merge(graphdef, model_state)

  restored_opt_dict = checkpointer.restore(os.path.join(ckpt_folder, 'optimizer_state'))
...
def validation_loop(
  ...
  for raw_tokens in val_loader:
    ...
    for k in per_val_step_logs.keys():
      if k not in logs:
        logs[k] = 0.0
      logs[k] += float(per_val_step_logs[k])
    n += 1
