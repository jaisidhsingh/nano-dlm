...
def main(cfg: Config):
    ...
    with jax.set_mesh(mesh):
        ...
        for step in range(cfg.train.max_steps):
            ...
            # get training data
            batch, batch_info = prepare_batch(
                raw_tokens, noise_schedule, cfg, rng_t, rng_mask, training=True
            )
            ...
            train_logs, param_logs = train_step(
                model, optimizer, noise_schedule, batch, labels
            )
            ...
            if cfg.exp.saving and step % cfg.exp.save_every == 0:
                save_checkpoint(
                    checkpointer, cfg, model, optimizer, metric_logger, step
                )
